...
class AkashiClient:
    ...
    def dakoku(self, dakoku_type):
        ...
        url = parse.urljoin(
            self.url,
            self.base_path + "/{}/stamps?token={}".format(self.company_id, self.token),
        )
    ...
    def get_stamps(self, start_date, end_date):
        ...
        url = parse.urljoin(
            self.url,
            self.base_path
            + "/{}/stamps?token={}&start_date={}&end_date={}".format(
                self.company_id,
                self.token,
                datetime.strftime(start_date, _date_format),
                datetime.strftime(end_date, _date_format),
            ),
        )
    ...
    def _request_api(self, req):
        ...
        try:
            ...
            logging.error(
                "return NG response from akashi. code:{} ,reason: {}".format(
